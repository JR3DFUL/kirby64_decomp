...
/* FACTORY: 379/403 [was noted 24/403], frame 0x130 vs the ROM's 0x120 -- 16 bytes of locals we
   ...
   Vectors at 0x20/0x2C/0x38/0x44/0x110, which is the same count this draft
   declares -- so the extra 16 bytes are the two scalars (`angle`, `objId`)
   that the ROM keeps in registers, not an extra temporary.  Moving them
   ahead of the matrices (LEVERS 12) does not help; they have to stop
   occupying stack at all. */
...
void func_801E49B8_ovl17(struct GObj *arg0) {
    ...
    dir.x = -gEntitiesNextPosXArray[omCurrentObj->objId];
